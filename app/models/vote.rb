class Vote < ApplicationRecord
  ...
  def count_vote_per_reservation
    current_user_id = User.find(user_id)
    current_user_voted_reservations = Vote.where(user_id: current_user_id, reservation_id: reservation.id)
  ...
  def not_vote_question_owner
    vote_reservation = Reservation.find(reservation_id)
    vote_question = vote_reservation.question
    if user == vote_question.user
