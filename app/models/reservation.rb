class Reservation < ApplicationRecord
  ...
  def reservation_time_check
    ...
    errors.add(:start_time, "が重複して登録されています。") if start_times.include?(self.start_time)
  end
  ...
  def rank_count_check
    ...
    errors.add(:rank, "が重複して登録されています。もう一度選択し直してください。") if existing_ranks.include?(self.rank) && self.rank != "default"
  end

  def rank_not_default_check
    @question = Question.find(self.question.id)
    errors.add(:rank, "が-になっています。") if self.rank == "default"
  end
